...
void backtracking(int n, vector<Node>& tree, vector<int>& order) {
	if (n == -1)
		return;

	backtracking(tree[n].left, tree, order);
	order.push_back(tree[n].key);
	backtracking(tree[n].right, tree, order);
}

bool isBinarySearchTree(vector<Node>& tree) {
	vector<int> order;
	backtracking(0, tree, order);
