...
//============================Functions===================================
int count_num(vector<int>& arr, int l, int r, int value)
{
	int count = 0;
	forbe(i, l, r)
		if (arr[i] == value)
...
int find_majority(vi& arr, int l, int r)
{
	if (l == r)
		return arr[l];

	int mid = l + (r - l) / 2;
	int left = find_majority(arr, l, mid);
	int right = find_majority(arr, mid+1, r);

	if (left == right)
		return left;

	int count_left = count_num(arr, l, r, left);
	int count_right = count_num(arr, l, r, right);
...
void solve()
{
	...
	int majority = find_majority(arr, 0, arr.size() - 1);
	int count_majority = count_num(arr, 0, arr.size() - 1, majority);
